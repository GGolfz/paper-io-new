...
function Game(id) {
  ...
  const grid = new Grid(consts.GRID_COUNT, (row, col, before, after) => {
    if (!!after ^ !!before) {
      ...
      if (filled === consts.GRID_COUNT * consts.GRID_COUNT)
        console.log(`[${new Date()}] FULL GAME`);
    }
  });
  this.id = id;
  this.addPlayer = (client, name) => {
    ...
    client.on("killNotify", (data) => {
      client.emit("killAlert", {
        killer: data.killer,
        other: data.other,
      });
    });
  ...
  function verifyPlayerLocations(fr, verify, resp) {
    ...
    for (const num in verify) {
      ...
      if (
        locs[num][0] !== verify[num][0] ||
        locs[num][1] !== verify[num][1] ||
        locs[num][2] !== verify[num][2]
      ) {
        resp(
          false,
          true,
          "P" + num + " " + string(locs[num]) + " !== " + string(verify[num])
        );
        return;
      }
    }
